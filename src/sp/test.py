...
def swing():
    ...
    if swinging:                         # ignore if already swinging to prevent overlap
        return
    swinging = True
    start = BASE_ANGLE + AIM_OFFSET      # where the bat starts (resting aim)
    peak = start + 100                   # how far the bat will travel in the swing (bigger = longer arc)
    step = 3                             # how many degrees to move each frame (smaller = smoother)
    a = start
...
def _unit(vx, vy):
    L = math.hypot(vx, vy)               # length of the vector (vx, vy)
...
def _bat_contact_details(ball_xy):
    ...
    close_enough = d_perp <= (BAT_W/2 + BALL_R)     # is the ball close enough to touch the bat rectangle?
